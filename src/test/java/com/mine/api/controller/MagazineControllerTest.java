...
class MagazineControllerTest {
    ...
    void getMyMagazines() throws Exception {
        given(magazineService.getMagazinesByUser("testuser")).willReturn(List.of(magazine));

        mockMvc.perform(get("/api/magazines"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].title").value("Test Magazine"));
    }
    ...
    void updateMagazine() throws Exception {
        ...
        doNothing().when(magazineService).updateMagazine(eq(1L), any(MagazineDto.UpdateRequest.class), eq("testuser"));

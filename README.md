...
### 🎯 교수님 멘토링 피드백 및 넥스트 스텝 (Backend Action Items)
최근 진행된 멘토링 회의록을 바탕으로 도출된 백엔드 필수 과제 우선순위입니다.

#### 🔥 1순위 (필수 / 서비스 신뢰도 직결)
**"AI 데이터의 출처(Source Link) 추적 및 요약 고지 시스템 구축"**
- [ ] **DB 스키마 추가**: `Paragraph` 엔티티 등에 원문 URL을 저장할 수 있는 `sourceUrl` 컬럼 생성
- [ ] **AI 프롬프트 수정**: Python 서버(`magazine_maker.py`)가 웹 검색 요약 시 원본 링크를 추출해 JSON에 포함
- [ ] **면책 조항(Disclaimer) 제공**: 프론트엔드가 "자체 알고리즘(ChatGPT 등)으로 요약된 결과입니다"를 띄울 수 있도록 API 메타데이터 추가

#### 🏃‍♂️ 2순위 (코어 UX / 딜레이 문제 해결)
**"매거진 사전 생성(Pre-generation) 자동화 및 추천 API 고도화"**
- [ ] **배치/스케줄링 로직 도입**: `@Scheduled` 등을 이용해 서버가 유휴 상태일 때 다양한 매거진을 미리 생성하여 DB에 적재
- [ ] **캐싱 및 탐색 API**: 유저가 기다릴 필요 없이 즉시 읽을 수 있는 '사전 생성 매거진 추천 리스트' 반환 API 구현

#### 🛠 3순위 (AI 퀄리티 향상)
**"AI 프롬프트 엔지니어링 및 표지 텍스트 합성 처리"**
- [ ] **다양성 튜닝**: 단일 객체에 매몰되지 않도록 `magazine_maker.py`의 프롬프트 세부 지시문 튜닝 (예: 여러 캐릭터 생성명령어)
- [ ] **표지 타이포그래피 (Image AI)**: 썸네일 생성 시 프롬프트에 `Typography` 기법을 명시하거나, 백엔드에서 원본 이미지 위에 제목을 합성한 뒤 S3에 업로드

#### 💡 백엔드 핵심 어필 포인트 (공유성)
- "큐레이션이 손쉽게 공유된다"는 강점을 살리기 위해, 매거진 공유 URL 동작 시 메신저(카카오톡, 슬랙)에 이미지와 제목이 뜨도록 **OG 태그(Open Graph) 동적 생성용 API** 최적화 및 안정성 점검
